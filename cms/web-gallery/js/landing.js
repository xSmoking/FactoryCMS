...
var Hash = Class.create(Enumerable, (function() {
    ...
    return{initialize: function(B) {
            ...
            return this.map(function(D) {
                var C = encodeURIComponent(D.key), B = D.value;
...
if (Prototype.BrowserFeatures.XPath) {
    document._getElementsByXPath = function(F, A) {
        ...
        for (var B = 0, D = E.snapshotLength; B < D;
                B++) {
...
Object.extend(Selector, {_cache: {}, xpath: {descendant: "//*", child: "/*", adjacent: "/following-sibling::*[1]", laterSibling: "/following-sibling::*", tagName: function(A) {
        ...
        }, className: "[contains(concat(' ', @class, ' '), ' #{1} ')]", id: "[@id='#{1}']", attrPresence: function(A) {
            A[1] = A[1].toLowerCase();
            ...
            }}}, criteria: {tagName: 'n = h.tagName(n, r, "#{1}", c);      c = false;', className: 'n = h.className(n, r, "#{1}", c);    c = false;', id: 'n = h.id(n, r, "#{1}", c);           c = false;', attrPresence: 'n = h.attrPresence(n, r, "#{1}", c); c = false;', attr: function(A) {
            A[3] = (A[5] || A[6]);
            ...
            if (D) {
                for (var B = A.childNodes, E = B.length - 1, C = 1;
                        E >= 0; E--) {
                    var F = B[E];
...
if (Prototype.Browser.IE) {
    Object.extend(Selector.handlers, {concat: function(B, A) {
...
Event.Methods = (function() {
    ...
    return{isLeftClick: function(C) {
            ...
            return{x: C.pageX || (C.clientX + (document.documentElement.scrollLeft || document.body.scrollLeft)), y: C.pageY || (C.clientY + (document.documentElement.scrollTop || document.body.scrollTop))}
        }, pointerX: function(C) {
...
Element.addMethods();
if (window.Prototype) {
    Ajax.Responders.register({onCreate: function(A, C) {
            var B = Cookie.get("JSESSIONID");
            if (B) {
                if (typeof A.options.requestHeaders == "object") {
                    A.options.requestHeaders["X-App-Key"] = B
                } else {
                    A.options.requestHeaders = {"X-App-Key": B}
                }
            }
...
var Cookie = {set: function(C, D, B) {
        ...
        return(document.cookie = escape(C) + "=" + escape(D || "") + "; path=/" + A)
    }, get: function(A) {
...
var HabboView = {_callbacks: [], add: function(A) {
...
var Rounder = function() {
    ...
    var B = function(M, N) {
        ...
        M.select(".title").each(function(Q) {
            if (!Q.parentNode.className || Q.parentNode.className != "rounded-container") {
...
var Utils = {getPageSize: function() {
        var D, A;
        ...
        new PrettyTimer(A, function(D) {
            C.update(D)
        }, {showDays: false, showMeaningfulOnly: false, localizations: {hours: L10N.get("time.hours") + " ", minutes: L10N.get("time.minutes") + " ", seconds: L10N.get("time.seconds")}, endCallback: B})
    }};
var RememberMeUI = {};
...
SpeechBubble.prototype = {initialize: function() {
        this.template = '<div class="cb bubble" id="#id#"><div class="bt"><div></div></div><div class="i1"><div class="i2"><div class="i3">#content#</div></div><div class="spike"></div></div><div class="bb"><div></div></div></div>';
        this.bubbles = []
    }, add: function(E, B, A, D, C) {
        this.bubbles.push({id: E, target_id: B, x: A, y: D, text: C})
    }, render: function() {
        for (var C = 0; C < this.bubbles.length; C++) {
            var A = this.bubbles[C];
            var B = this.template.replace("#id#", A.id);
            B = B.replace("#content#", A.text);
            var D = this.guessBubbleWidth(A.text, A.target_id);
            Element.insert(A.target_id, B);
            var E = $(A.id);
            E.setStyle({width: D + "px", left: Math.round(A.x - D / 2 - 3) + "px", top: (A.y - 24) + "px"})
        }
    }, guessBubbleWidth: function(E, B) {
        var D = document.createElement("span");
        D.id = "wtemp";
        D.style.visibility = "hidden";
        D.className = "bubble";
        var A = document.createTextNode(E.unescapeHTML());
        D.appendChild(A);
        $(B).insert(D);
        var C = $("wtemp").getWidth();
        $("wtemp").remove();
        return C + 22
    }};